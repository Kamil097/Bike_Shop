...
namespace BikeShopREST.Controllers
{
    ...
    public class AddressController : Controller
    {
        ...
        public IActionResult UpdateAddress([FromBody] AddressDto addressUpdate, int addressId) 
        {
            if (addressUpdate == null)
                return BadRequest(ModelState);
            if(addressId != addressUpdate.Id)
                return BadRequest(ModelState);
            if (!_addressRepository.AddressExists(addressId))
                return NotFound();
            if (!ModelState.IsValid)
            ...
            var addressMap = _mapper.Map<Address>(addressUpdate);
        ...
        public IActionResult DeleteAddress(int addressId) 
        {
            if (!_addressRepository.AddressExists(addressId))
                return NotFound();
            var addresDelete = _addressRepository.GetAddress(addressId);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            if (!_addressRepository.DeleteAddress(addresDelete))
            {
